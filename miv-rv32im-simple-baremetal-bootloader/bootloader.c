...
/*Sectors are 64K bytes*/
#define FLASH_SECTOR_SIZE   65536
/*This is an 8MB part with 128 sectors of 64KB*/
#define FLASH_SECTORS       128
/*We will use the 4K blocks for this example*/
#define FLASH_BLOCK_SIZE    4096
/*Write segment size*/
#define FLASH_SEGMENT_SIZE  256
...
static void Bootloader_JumpToApplication();
...
/*
 * Base address of the memory where user provided executable will be loaded.
 */
#define USER_EXECUTABLE_ADDRESS    0x80000000
...
const uint8_t g_greeting_msg[] =
        "\r\n\r\n\
===========================================================================\r\n\
                    Microsemi RISC-V Boot Loader \r\n\
===========================================================================\r\n\
 This boot loader provides the following features:\r\n\
 - Load a program into memory using the YModem file transfer protocol.\r\n\
 - Write a program into the board's SPI flash. The executable must first be\r\n\
   loaded into the board's memory using the YModem file transfer\r\n\
   protocol.\r\n\
 - Load a program from SPI flash into external memory and launch the\r\n\
   program.\r\n\
...
---------------------------------------------------------------------------\r\n\
 ...
 Type 1 to start Ymodem transfer to memory\n\r\
 Type 2 to copy program from DDR to flash \n\r\
 Type 3 to copy program from flash to memory \n\r\
...
---------------------------------------------------------------------------\r\n\
 ...
 Toggle DIP switch 8 and reset the board if you wish to execute the boot loader\r\
...
---------------------------------------------------------------------------\r\n\
 ...
 Toggle DIP switch 8 and reset the board if you wish to automatically execute\r\
...
const uint8_t g_load_executable_msg[] =
        "\r\n\
---------------------------------------------------------------------------\r\n\
 Loading application from SPI flash into memory.\r\n";
...
const uint8_t g_ymodem_info_msg[] =
        "\r\n\
---------------------------------------------------------------------------\r\n\
...
const uint8_t g_write_data_flash[] =
        "\r\n\
---------------------------------------------------------------------------\r\n\
------------------ Writing SPI flash from memory --------------------\r\n\
This may take several minutes to complete if writing a large file.\r\n\
---------------------------------------------------------------------------\r\n\
 \r\n";

const uint8_t g_read_data_flash[] =
        "\r\n\
---------------------------------------------------------------------------\r\n\
------------------ Reading SPI flash into memory --------------------\r\n\
This will take several minutes to complete in order to read the full SPI flash\r\
 \ncontent.\r\n\
---------------------------------------------------------------------------\r\n\
 \r\n";
...
int main(void)
{
    ...
    while (wait_in_bl == 1)
    {
        ...
        if (rx_size > 0)
        {
            ...
            case '1':
                /*Receive file using YMODEM protocol*/
                file_size = rx_app_file((uint8_t*)USER_EXECUTABLE_ADDRESS);
                break;

            case '2':
                /*Write received program to flash memory*/
                write_program_to_flash((uint8_t *)USER_EXECUTABLE_ADDRESS,
                        file_size);
                readback_size = file_size;
                break;

            case '3':
                /*Read back the contents of flash memory*/
                read_program_from_flash((uint8_t *)USER_EXECUTABLE_ADDRESS,
                        readback_size);
                break;

            case '4':
                /*Jump to application in flash memory*/
                Bootloader_JumpToApplication();
                break;

            case '5':
                /*
                 * Sanity check on FLASH working
                 */
                test_flash();
                break;
            ...
            case '6':
                 ...
                 */
                mem_test((uint8_t *)USER_EXECUTABLE_ADDRESS);
                break;
            }
        }
    }
    ...
    /*
     * Execute the application file loaded in DDR memory.
     */
    UART_polled_tx_string(&g_uart, g_load_executable_msg);
    read_program_from_flash((uint8_t *)USER_EXECUTABLE_ADDRESS, (64 * 1024));
...
static int test_flash(void)
{
    ...
    uint8_t read_buff[FLASH_SEGMENT_SIZE];
    uint16_t status;
     ...
     */
    for (count = 0; count != FLASH_SECTORS; ++count)
    {
    ...
    spi_flash_read(flash_address, read_buff, FLASH_SEGMENT_SIZE);
    UART_polled_tx_string(&g_uart, read_buff);
    ...
    for (count = 0; count != 1000 /*32768*/; ++count)
    {
        ...
        {
            result = spi_flash_control_hw(SPI_FLASH_4KBLOCK_ERASE, flash_address,
                    NULL);
        }
...
static int write_program_to_flash(uint8_t *write_buf, uint32_t file_size)
{
    ...
    UART_polled_tx_string(&g_uart, g_write_data_flash);
     ...
     */
    {
        ...
        if (memcmp((uint8_t *)&flash_content, read_buffer, FLASH_SEGMENT_SIZE))
        {
            while (1)
            {
                /* Breakpoint here will trap write faults*/
            }
        }
        write_buf += FLASH_SEGMENT_SIZE;

        /* Step to the next 256 byte chunk */
        flash_address += FLASH_SEGMENT_SIZE;
        show_progress();
    }
...
static int read_program_from_flash(uint8_t *read_buf, uint32_t read_byte_length)
{
    ...
    UART_polled_tx_string(&g_uart, g_read_data_flash);
...
static uint32_t rx_app_file(uint8_t *dest_address)
{
    ...
    uint32_t g_rx_size = 1024 * 64;

    UART_polled_tx_string(&g_uart, g_ymodem_info_msg);
...
static void Bootloader_JumpToApplication()
{
    ...
    /* Start executing from the top of memory: */
    __asm volatile("lui ra,0x80000");
    __asm volatile("fence");
    __asm volatile("fence.i");
    __asm volatile("jr ra");
